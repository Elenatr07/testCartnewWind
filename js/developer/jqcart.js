    ...
    var actions = {
           ...
           orderPreview = '<div class="trip_order" id="trip_order"> <button type="button" class="edit" id="edit_trip"><img src="img/pen.svg" alt=""></button>   '
            ...
            for (key in cartData) {
                if (cartData.hasOwnProperty(key)) {
                    ...
                    orderPreview += '<div class="way_pass"><h4>Passengers: </h4><pre> ' + cartData[key].pass + ' </div>';
                    orderPreview += '<div class="way_info"><p> '+ cartData[key].date + ' <span class="from_time"> from </span> ' + cartData[key].timedeparture + '</p> </div>';
                    orderPreview += '<div class="way_info" data-value="'+ cartData[key].hours +'" id="trip_hours"><h4> Duration: </h4> '+ cartData[key].hours +'<h4 class="h">h</h4></div>';
        ...
        //openTripview ToyotaCommuter start
                ...
                for (key in cartData) {
                    if (cartData.hasOwnProperty(key)) {
                        ...
                        openTripview += '<p class="time_trip">'+ cartData[key].hours + '<span class="h">h</span> </p>'
                    }
                }
           ...
           for (key in cartData){
            if (cartData.hasOwnProperty(key)){
                ...
                openTripview += ' <p >IDR <span class="costView" id="costViewCommuter"> '+ cartData[key].pricecommuter * cartData[key].hours +' </span>*</p>';
             }} 
                openTripview += ' </div>';
                openTripview += '</div>';
                openTripview += '</div>';
                openTripview += ' <div class="trip_order_distance_block">';
                openTripview += '<div class="trip_order_distance_from"><span>Your Location in </span><pre> '+ cartData[key].from + '</div>';
                openTripview += '<div class="trip_order_distance_to"><h4> Your Desired Location</h4></div>';
                openTripview += ' </div>';
                openTripview += '</div>';
      ...
      for (key in cartData){
       if (cartData.hasOwnProperty(key)){
           ...
           openTripview += ' <p >IDR <span class="costView" id="costViewHiAce"> '+ cartData[key].pricepremio * cartData[key].hours +'</span>*</p>';
        }} 
           openTripview += ' </div>';
           openTripview += '</div>';
...
if (cartData.hasOwnProperty(key)){
    ...
    openTripview += ' <p >IDR <span class="costView" id="costViewInnova"> '+ cartData[key].pricereborn * cartData[key].hours +'</span>*</p>';
 }} 
    openTripview += ' </div>';
    openTripview += '</div>';
...
//openTripview ToyotaInnova end 
       ...
       //     orderPreview += '<div class="jqcart-subtotal">Итого: <strong>' + (subtotal - discountSum) + '</strong> ' + opts.currency + '</div>';
        ...
        openCart_id: function(){
            $(location).attr('href','./box_id.html')
        },
        hideCart: function () {
    ...
    var methods = {
        ...
        openCart_id: actions.openCart_id,
